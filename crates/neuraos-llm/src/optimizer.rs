...
/// Optimizer tracks per-model stats and selects the best model for given requirements.
...
impl LlmOptimizer {
    pub fn new() -> Self {
        ...
        // Seed with known model metadata
        opt.seed_defaults();
        opt
    }
    ...
    fn seed_defaults(&self) {
        let defaults: &[(&str, u64, f64, f32)] = &[
            // (model_id, latency_ms, cost_per_1k, quality)
    ...
    pub fn select(&self, reqs: &ModelRequirements) -> ModelSelection {
        ...
        for entry in self.models.iter() {
            ...
            if let Some(max_cost) = reqs.max_cost_per_1k_tokens {
                if stats.avg_cost_per_1k() > max_cost {
                    continue;
                }
            }
            if let Some(max_lat) = reqs.max_latency_ms {
                if stats.avg_latency_ms() > max_lat {
                    continue;
                }
            }
            if let Some(min_q) = reqs.min_quality_score {
                if stats.avg_quality() < min_q {
                    continue;
                }
            }
            if let Some(pref) = &reqs.preferred_provider {
                if !model_id.starts_with(pref.as_str()) {
                    continue;
                }
            }
    ...
    /// Update stats after a successful call.
    ...
    /// Update stats after a failed call.
...
/// Simple pseudo-random normal variate using Box-Muller approximation.
